...
# - IP masking enabled for security
...
locals {
  ...
  # Example: rg-finrisk-prod, kv-finrisk-prod
...
module "observability" {
  ...
  log_analytics_daily_quota_gb = 10 # Higher quota for production

  # Application Insights configuration
  app_insights_name         = "appi-${local.naming_prefix}"
  application_type          = "web"
  sampling_percentage       = 100 # Full sampling in production
  app_insights_daily_cap_gb = 5   # Higher cap for production

  # Production settings: Security hardening
  disable_ip_masking            = false # Mask IPs for privacy/compliance
  local_authentication_disabled = true  # Require AAD auth
  internet_ingestion_enabled    = true
  internet_query_enabled        = true

  # Availability test (enabled in production)
...
module "container_registry" {
  ...
  # Production: Standard SKU (better performance than Basic)
  sku = "Standard"

  # Production: Public access enabled (consider private endpoint for enhanced security)
  public_network_access_enabled = true

  # Retention and trust policies
  retention_enabled    = true  # Clean up old images
  trust_policy_enabled = false # Enable if using signed images
...
module "key_vault" {
  ...
  # Production: Premium SKU for HSM-backed keys (enhanced security)
  # Change to "standard" if HSM is not required
  sku_name = "standard"
  ...
  # Production: Consider private endpoint for enhanced security
  public_network_access_enabled = true

  # Production: Consider network ACLs for enhanced security
  ...
  secrets = {
    ...
    # az keyvault secret set --vault-name kv-finrisk-prod --name RISKSHIELD-API-KEY --value <actual-key>
  }
...
module "container_app" {
  ...
  # Production: No VNet integration (Azure-managed network)
  ...
  # Production: Zone redundancy enabled for high availability
  ...
  # Production: Always on (min_replicas = 2 for high availability)
  # No scale to zero in production
